...
- **Freshness vs readiness**: `hk sync --check` answers whether ledger work changed after the last checkpoint. `hk ready` is the ledger-backed Harness Kit lifecycle readiness gate. `mise run sync-check` validates committed HK `.ai/hk/<work-id>/` export packages when present. New generated projects use native task and verification-route contracts instead of plan artifacts.
